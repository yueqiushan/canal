...
public class DefaultRedisRepository<T> extends AbstractRedisRepository<T> implements RedisUniqueKeyRepository<T>,
    ...
    public Optional<T> findOne(CombineKey combineKey) {
        ...
        String suffix =
                entries.stream().map(Entry::getName).collect(Collectors.joining(RedisConstants.SEPARATOR1));
        String key = RedisUtils.key(canalConfig.getSchema(), tableName(), suffix);
        String hashKey =
                entries.stream().map(Entry::getValue).map(Object::toString).collect(Collectors.joining(RedisConstants.SEPARATOR1));
    ...
    public List<T> findAll(UniqueKeyPro uniqueKeyPro) {
        String key = RedisUtils.key(canalConfig.getSchema(), tableName(), uniqueKeyPro.getName());
